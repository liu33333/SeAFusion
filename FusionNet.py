...
class ConvBnLeakyRelu2d(nn.Module):  # 卷积层+BN层+LeakyRelu层（负区域内采用ax代替0，a=0.01）
    # convolution
    # batch normalization
    # leaky relu
...
class ConvBnTanh2d(nn.Module): # 卷积层+BN层+tanh层（双曲正切函数作为激活函数）
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, stride=1, dilation=1, groups=1):
...
class DenseBlock(nn.Module):  # 稠密块
    ...
    def forward(self, x):
        x = torch.cat((x, self.conv1(x)), dim=1)  # 这里把第一个卷积层的输出和输入拼接在一起，作为第二个卷积层的输入
        x = torch.cat((x, self.conv2(x)), dim=1)
        # x = torch.cat((x, self.conv3(x)), dim=1)
        return x


class RGBD(nn.Module):  # RGBD模块 RGB+depth
    ...
    def forward(self, x): #x1:主干，两个密集块加一个卷积层，x2：梯度分支，sobel算子+卷积
        x1 = self.dense(x)
        x1 = self.convdown(x1)
        x2 = self.sobelconv(x)
        x2 = self.convup(x2)
